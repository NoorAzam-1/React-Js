...
const INITIAL_NOTIFICATIONS = [
  { id: 1, title: "Overdue Follow-up", msg: "Komal Deshmukh's phone follow-up is overdue.", time: "2 hrs ago", unread: true, category: "warning" },
  { id: 2, title: "New Lead Assigned", msg: "New Lead 'Nikhil Mehra' assigned to Amit Sharma via Facebook API.", time: "4 hrs ago", unread: true, category: "info" },
  { id: 3, title: "Campaign Completed", msg: "Pre-Monsoon Dining Campaign reached 4,500 contacts successfully.", time: "1 day ago", unread: false, category: "success" }
];
...
export default function App() {
  ...
  const startLiveCampaignSending = (campaignId, size = 4320) => {
    if (liveSendingState.active) {
      showToast("Another WhatsApp campaign is actively processing queues right now.", "error");
      return;
    }
    ...
    showToast("⚡ WhatsApp Gateway Connected. Anti-Spam throttling engaged.", "success");
    ...
    sendingIntervalRef.current = setInterval(() => {
      ...
      if (count >= size) {
        ...
        setLiveSendingState(prev => {
          ...
          setCampaigns(oldCampaigns => oldCampaigns.map(c => {
            if (c.id === campaignId) {
              return {
                ...c,
                status: "Completed",
                ...
                deliveryPercent: Number(((finalDelivered / size) * 100).toFixed(1)),
  ...
  const createTemplate = (e) => {
    ...
    showToast("Template registered successfully and approved by Meta API!", "success");
  };
  ...
  const onDrop = (e) => {
    ...
    if (files.length > 0) {
      ...
      showToast(`Parsed spreadsheet: ${files[0].name} (Found 4,320 validated Indian phone contacts)`, "success");
    }
  };
  ...
  const stats = useMemo(() => {
    ...
    const totalSentCampaigns = campaigns.reduce((acc, curr) => acc + curr.sent, 0);
    const totalDeliveredCampaigns = campaigns.reduce((acc, curr) => acc + curr.delivered, 0);
    const totalReadCampaigns = campaigns.reduce((acc, curr) => acc + curr.read, 0);
    const totalRepliedCampaigns = campaigns.reduce((acc, curr) => acc + curr.replied, 0);
  ...
  const handleCreateLead = (e) => {
    ...
    showToast(`Lead for ${newLead.name} created and assigned!`, "success");
    setNotifications([
      { id: Date.now(), title: "Lead Created", msg: `${newLead.name} assigned to ${newLeadForm.assignedTo}`, time: "Just now", unread: true, category: "success" },
      ...notifications
    ]);
    setNewLeadForm({
      name: "", mobile: "", whatsapp: "", alternateMobile: "",
  ...
  const simulateIncomingWhatsApp = () => {
    ...
    const simNames = ["Neha Kapoor", "Rahul Jaiswal", "Sneha Chawla", "Devendra Dixit"];
    const simPhones = ["+91 90512 88123", "+91 93122 77411", "+91 98111 65332", "+91 94123 99882"];
    const simProducts = ["Nilkamal Goa Sofa Set with Cushion", "Nilkamal Akin Queen Bed", "Nilkamal Freedom Cabinet", "Nilkamal Rippon Sofa Unit"];
    ...
    const newLead = {
      ...
      whatsapp: randomPhone,
      alternateMobile: "",
      address: "Address verified automatically over WhatsApp webhook integration",
      city: "Mumbai",
      product: randomProduct,
      budget: randomBudget,
      source: "WhatsApp",
      ...
      timeline: [
        { date: new Date().toLocaleString(), user: "System (Webhook)", text: `Customer replied to Campaign. Automated lead registered and assigned to ${randomEmp}.` }
      ]
    };

    setLeads(prev => [newLead, ...prev]);
    setNotifications(prev => [
      { id: Date.now(), title: "⚡ Instant Webhook Auto-Reply", msg: `Interactive Campaign hit reply from ${randomName}. Transferred to ${randomEmp}'s desk.`, time: "Just now", unread: true, category: "success" },
      ...prev
    ]);

    showToast(`⚡ Response captured: Lead '${randomName}' assigned!`, "success");
  };
  ...
  const assignChatEmployee = (chatId, employeeName) => {
    setWhatsappChats(prev => prev.map(c => {
      if (c.id === chatId) { return { ...c, assignedTo: employeeName }; }
      return c;
    }));
    showToast(`Chat conversation assigned to ${employeeName}`, "success");
  };
  ...
  const triggerCommunication = (clientName, platform) => {
    showToast(`Initiating outbound direct message path via ${platform} to ${clientName}!`, "success");
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex font-sans antialiased overflow-x-hidden selection:bg-blue-600 selection:text-white">
      ...
      <aside className={`bg-white border-r border-slate-100 flex flex-col transition-all duration-300 ease-in-out shrink-0 z-30 ${
        ...
        <nav className="flex-1 p-3 space-y-1">
          {[
            { id: "campaigns", label: "Bulk Campaigns", icon: Megaphone, badge: 1, specialGlow: true },
            { id: "dashboard", label: "Dashboard", icon: LayoutDashboard },
            { id: "leads", label: "Lead Manager", icon: Users, badge: leads.length },
            { id: "whatsapp", label: "WhatsApp Chat", icon: MessageSquare, badge: stats.unreadWaChats },
            { id: "employees", label: "Employees", icon: UserCheck },
            { id: "followups", label: "Follow-Ups", icon: Calendar, badge: stats.pendingFollowups + stats.missedFollowups },
            { id: "analytics", label: "Analytics Hub", icon: BarChart3 },
            { id: "orders", label: "Converted Sales", icon: ShoppingBag },
        ...
        {!isSidebarCollapsed && (
          <div className="p-4 m-3 bg-gradient-to-tr from-slate-50 to-blue-50/30 border border-slate-100 rounded-2xl">
            <div className="flex items-center gap-2.5">
              ...
              <div className="truncate">
                <p className="text-xs font-semibold text-slate-900">Shantanu Adhikari</p>
                <p className="text-[10px] text-slate-400 truncate">Store Admin Manager</p>
              </div>
            </div>
          </div>
        ...
        <header className="sticky top-0 bg-white/80 backdrop-blur-md border-b border-slate-100 px-6 py-3.5 flex items-center justify-between z-20">
          <div className="flex items-center gap-4">
            <div>
              ...
              <h2 className="text-lg font-bold text-slate-900 capitalize tracking-tight mt-0.5">
                {activeTab === "campaigns" ? "WhatsApp Bulk Campaigns Engine" : `${activeTab}space`}
              </h2>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div className="relative max-w-xs hidden md:block">
              ...
              <input
                ...
                onChange={(e) => {
                  setSearchQuery(e.target.value);
                  if (activeTab !== "leads" && activeTab !== "campaigns") setActiveTab("leads");
                }}
                className="pl-9 pr-4 py-2 text-xs w-64 bg-slate-50 border border-slate-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:bg-white transition-all text-slate-700"
              />
            </div>
        ...
        <main className="flex-1 p-6 space-y-6">
          ...
          {activeTab === "campaigns" && (
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-r from-emerald-500/10 via-teal-500/5 to-transparent border border-emerald-500/20 p-5 rounded-3xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 font-extrabold text-[9px] uppercase rounded">Enterprise</span>
                    <span className="text-xs text-slate-500">Official Meta WhatsApp API Portal</span>
                  </div>
                  <h3 className="text-xl font-extrabold text-slate-900 tracking-tight">Safely Reach 4000+ Verified Buyers Instantly</h3>
                  <p className="text-xs text-slate-500">A/B Testing, dynamic customer personalization fields, and live delivery webhook graphs.</p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => {
              ...
              <div className="flex gap-1.5 bg-white p-1.5 border border-slate-100 rounded-2xl text-xs font-bold text-slate-500 max-w-2xl">
                {[
                  { id: "overview", label: "Campaign Dashboard", icon: LayoutDashboard },
                  { id: "builder", label: "Interactive Campaign Builder", icon: Sliders },
                  { id: "templates", label: "Meta Approved Templates", icon: MessageSquare },
                  { id: "analytics", label: "Deep Flow Analytics", icon: BarChart3 },
                  { id: "limits", label: "API Throttling & Limits", icon: ShieldCheck }
                ].map(ctb => {
                  const Icon = ctb.icon;
                ...
                <div className="bg-slate-900 text-white p-5 rounded-3xl border border-slate-800 shadow-xl space-y-4 animate-pulse-glow">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h4 className="font-black text-sm text-white">AUTOPILOT SENDING: {campaigns.find(c => c.id === liveSendingState.campaignId)?.name}</h4>
                          <span className="w-2 h-2 bg-emerald-500 rounded-full animate-ping" />
                        </div>
                        <p className="text-xs text-slate-400">Meta API rate-limit delays in process: 1 message per 2.5s.</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <button
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3 pt-3 border-t border-slate-800 text-xs">
                    <div className="bg-slate-800/40 p-3 rounded-2xl border border-slate-800">
                      <span className="text-slate-400 font-bold block text-[9px] uppercase">Sent Bulk Packages</span>
                      <strong className="text-white text-lg mt-0.5 block">{liveSendingState.sentCount} <span className="text-slate-500 text-xs">/ {liveSendingState.totalCount}</span></strong>
                    </div>
                    <div className="bg-slate-800/40 p-3 rounded-2xl border border-slate-800">
                      <span className="text-emerald-400 font-bold block text-[9px] uppercase">API Delivered</span>
                      <strong className="text-emerald-400 text-lg mt-0.5 block">{liveSendingState.deliveredCount}</strong>
                    </div>
                    <div className="bg-slate-800/40 p-3 rounded-2xl border border-slate-800">
                      <span className="text-sky-400 font-bold block text-[9px] uppercase">Read Rate</span>
                      <strong className="text-sky-400 text-lg mt-0.5 block">{liveSendingState.readCount}</strong>
                    </div>
                    <div className="bg-slate-800/40 p-3 rounded-2xl border border-slate-800">
                      <span className="text-amber-400 font-bold block text-[9px] uppercase">Client Replied</span>
                      <strong className="text-amber-400 text-lg mt-0.5 block">{liveSendingState.repliedCount}</strong>
                    </div>
                    <div className="bg-slate-800/40 p-3 rounded-2xl border border-slate-800">
                      <span className="text-rose-400 font-bold block text-[9px] uppercase">Meta Failed / Rejected</span>
                      <strong className="text-rose-400 text-lg mt-0.5 block">{liveSendingState.failedCount}</strong>
                    </div>
                  </div>
              ...
              {campaignTab === "overview" && (
                <>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
                    {[
                      { title: "Active Campaigns", val: campaigns.length, percent: "Bulk Broadcasts", color: "blue", icon: Megaphone },
                      { title: "API Outbox Blasts", val: stats.totalSentCampaigns, percent: "Messages Sent", color: "indigo", icon: Send },
                      { title: "Delivered Webhook", val: stats.totalDeliveredCampaigns, percent: "98.2% avg success", color: "emerald", icon: CheckCheck },
                      { title: "Reads Detected", val: stats.totalReadCampaigns, percent: "88% avg open rate", color: "sky", icon: MessageSquare },
                      { title: "Customer Replies", val: stats.totalRepliedCampaigns, percent: "High client response", color: "amber", icon: Zap },
                      { title: "Meta Rejected", val: 208, percent: "Invalid numbers", color: "red", icon: AlertTriangle }
                    ].map((card, index) => {
                      const Icon = card.icon;
                      return (
                        <div key={index} className="bg-white border border-slate-100 p-4 rounded-2xl shadow-sm transition-all hover:shadow-md">
                          <div className="flex justify-between items-center">
                            ...
                            <div className={`p-1 rounded-lg ${
                              ...
                              card.color === "amber" ? "bg-amber-50 text-amber-600" :
                              card.color === "red" ? "bg-rose-50 text-rose-600" : "bg-blue-50 text-blue-600"
                            }`}>
                              <Icon className="w-4 h-4" />
                            </div>
                          </div>
                          <strong className="text-xl font-black text-slate-900 mt-2 block">{card.val.toLocaleString()}</strong>
                          <span className="text-[10px] text-slate-400 block mt-1">{card.percent}</span>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <div className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-sm">
                    <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                      <div>
                        <h4 className="font-extrabold text-slate-900 text-sm">Campaign Blasting Registers</h4>
                        <p className="text-xs text-slate-400">Past and active bulk message records dispatched from this branch.</p>
                      </div>
                    </div>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-slate-100/50 text-xs text-slate-600">
                          {campaigns.map(cmp => (
                            <tr key={cmp.id} className="hover:bg-slate-50/40 transition-colors">
                              <td className="p-4 pl-6">
                                <span className="font-bold text-slate-900 text-sm block">{cmp.name}</span>
                                <span className="text-[10px] text-slate-400 block mt-0.5">Dispatched: {cmp.sentDate} • By {cmp.createdBy}</span>
                              </td>
                              <td className="p-4">
                                <span className="font-semibold text-slate-700 block">{cmp.template}</span>
                                <span className="text-[9px] bg-slate-100 px-1 py-0.5 rounded text-slate-500 inline-block mt-1">META_APPROVED_SANDBOX</span>
                              </td>
                              <td className="p-4">
                                <span className="text-slate-500 font-semibold">{cmp.media}</span>
                              </td>
                              ...
                              <td className="p-4">
                                <div className="flex items-center gap-2">
                                  <span className="font-bold text-sky-600">{cmp.replyRate || 0}%</span>
                                  <span className="text-[10px] text-slate-400">({cmp.replied} chats back)</span>
                                </div>
                              </td>
                              <td className="p-4">
                                <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold ${
                                  cmp.status === "Completed" ? "bg-emerald-50 text-emerald-700" :
                                  cmp.status === "Sending" ? "bg-blue-50 text-blue-700 animate-pulse" :
                                  cmp.status === "Paused" ? "bg-amber-50 text-amber-700" :
                                  "bg-slate-100 text-slate-500"
                                }`}>
                                  {cmp.status}
                                </span>
                              ...
                              <td className="p-4 pr-6 text-right">
                                <div className="flex justify-end gap-1.5">
                                  {cmp.status === "Draft" && (
                                    <button
                                      onClick={() => startLiveCampaignSending(cmp.id, cmp.audienceSize || 4320)}
              ...
              {campaignTab === "builder" && (
                <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm max-w-4xl mx-auto">
                  <div className="flex justify-between items-center pb-4 border-b border-slate-100 mb-6">
                    <div>
                      <h3 className="font-bold text-slate-900 text-base">Step-by-Step Meta Bulk Campaign Builder</h3>
                      <p className="text-xs text-slate-400">Personalize variables, configure limits, choose channels and blast instantly.</p>
                    </div>
                  ...
                  <div className="flex gap-1 mb-6">
                    {[1, 2, 3, 4, 5, 6].map(st => (
                      <div 
                        key={st} 
                        className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${
                          st <= campaignWizardStep ? "bg-emerald-500" : "bg-slate-100"
                        }`} 
                      />
                    ))}
                  </div>
                  ...
                  <form onSubmit={(e) => {
                    ...
                    if (campaignWizardStep === 6) {
                      ...
                      const newCmp = {
                        ...
                        audienceSize: newCampaignForm.targetType === "all" ? 4320 : 1850,
                        status: "Sending",
                        sent: 0, delivered: 0, read: 0, replied: 0, failed: 0, clicked: 0,
                        template: templates.find(t => t.id === newCampaignForm.selectedTemplateId)?.name || "Clearance Sofa Promo",
                    ...
                    {campaignWizardStep === 1 && (
                      <div className="space-y-4 text-xs">
                        <h4 className="font-extrabold text-slate-800 text-sm">Target Audience Definition</h4>
                        
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          {[
                            { id: "all", label: "Broadcast All Customers (4,320 contacts)", desc: "Blasts every contact registered inside Nilkamal repository." },
                            { id: "segment", label: "Dynamic Segmentation Criteria", desc: "Filter by city, product tags or recent walk-in events." },
                            { id: "upload", label: "Upload Customer CSV Spreadsheet", desc: "Instantly drop Excel datasets with contact numbers." }
                          ].map(t => (
                            <div
                              key={t.id}
                              onClick={() => setNewCampaignForm({...newCampaignForm, targetType: t.id})}
                            ...
                            >
                              <strong className="text-slate-900 block font-bold">{t.label}</strong>
                              <span className="text-slate-400 block mt-1.5 leading-snug">{t.desc}</span>
                            </div>
                          ))}
                        </div>
                        ...
                        {newCampaignForm.targetType === "segment" && (
                          <div className="bg-slate-50 border border-slate-100 p-4 rounded-2xl space-y-3">
                            <h5 className="font-bold text-slate-800">Filter Segment Parameters</h5>
                            <div className="grid grid-cols-2 gap-3">
                              <div>
                                <label className="text-slate-400 font-semibold block mb-1">Filter by Showroom Location</label>
                                <select 
                                  value={newCampaignForm.selectedSegmentFilter}
                                  onChange={(e) => setNewCampaignForm({...newCampaignForm, selectedSegmentFilter: e.target.value})}
                              ...
                              <div>
                                <label className="text-slate-400 font-semibold block mb-1">Lead Segment Filter Status</label>
                        ...
                        {newCampaignForm.targetType === "upload" && (
                          <div 
                            ...
                            className={`border-2 border-dashed rounded-3xl p-8 text-center transition-all ${
                              isDragging ? "border-emerald-500 bg-emerald-50/20" : "border-slate-200 bg-slate-50/50"
                            }`}
                          >
                            <Upload className="w-8 h-8 text-slate-400 mx-auto mb-2" />
                            <strong className="text-slate-700 block font-bold text-xs">Drag and Drop Customer Database .CSV File Here</strong>
                            <span className="text-slate-400 block text-[10px] mt-1">Accepts CSV, XLSX files with mobile columns (Limit: 10,000 records)</span>
                            ...
                            {draggedFile ? (
                              <div className="mt-4 p-2.5 bg-emerald-50 border border-emerald-100 inline-block rounded-xl text-[11px] font-bold text-emerald-800">
                                Selected: {draggedFile.name} (4,320 Contacts parsed)
                              </div>
                            ) : (
                              <button 
                                type="button"
                                onClick={() => showToast("Simulating file chooser window click.", "info")}
                    ...
                    {campaignWizardStep === 2 && (
                      <div className="space-y-4 text-xs">
                        <h4 className="font-extrabold text-slate-800 text-sm">Message Content Template Selection</h4>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {templates.map(tmp => {
                            const isSelected = newCampaignForm.selectedTemplateId === tmp.id;
                            return (
                              <div
                                key={tmp.id}
                                onClick={() => setNewCampaignForm({...newCampaignForm, selectedTemplateId: tmp.id})}
                                className={`p-4 border rounded-2xl cursor-pointer text-left transition-all ${
                                  isSelected 
                              ...
                              >
                                <div className="flex justify-between items-start pb-2 border-b border-slate-100 mb-2">
                                  <div>
                                    <strong className="text-slate-950 font-bold text-xs">{tmp.name}</strong>
                                    <span className="text-slate-400 block text-[9px] mt-0.5">{tmp.category} • {tmp.language}</span>
                                  </div>
                    ...
                    {campaignWizardStep === 3 && (
                      <div className="space-y-4 text-xs">
                        <h4 className="font-extrabold text-slate-800 text-sm">Multi-Media Brochures Selection</h4>
                        <p className="text-slate-400">Meta API supports attaching high-res PDF catalogues, sofa clearance offer graphics, or videos.</p>
                        
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          {[
                            { name: "Nilkamal_Sofa_Sale_Banner.png", size: "1.2 MB", type: "Image" },
                            { name: "Premium_Teakwood_Catalog_2026.pdf", size: "4.8 MB", type: "PDF Document" },
                            { name: "Freedom_Resin_Cupboard_Showreel.mp4", size: "12.4 MB", type: "Video" },
                            { name: "None (Text Only Broadcast)", size: "0 KB", type: "Plain Text" }
                          ].map((med, i) => {
                            const isSelected = newCampaignForm.attachedMedia === med.name;
                            return (
                              <div
                                key={i}
                                onClick={() => setNewCampaignForm({...newCampaignForm, attachedMedia: med.name})}
                              ...
                              >
                                ...
                                <strong className="text-slate-800 block truncate">{med.name}</strong>
                                <span className="text-slate-400 block text-[10px] mt-1">{med.type} • {med.size}</span>
                              </div>
                            );
                          })}
                        </div>
                    ...
                    {campaignWizardStep === 4 && (
                      <div className="space-y-4 text-xs">
                        <h4 className="font-extrabold text-slate-800 text-sm">Anti-Spam & Delivery Safeguard Settings</h4>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="bg-slate-50 border border-slate-100 p-4 rounded-2xl space-y-3">
                            <h5 className="font-bold text-slate-800">Dispatch Calendar Options</h5>
                            <div className="space-y-2">
                              <label className="flex items-center gap-2 cursor-pointer font-semibold text-slate-700">
                                <input 
                                  type="radio" 
                                  name="sched" 
                                  checked={newCampaignForm.scheduledTime === "Send Immediately"}
                                  onChange={() => setNewCampaignForm({...newCampaignForm, scheduledTime: "Send Immediately"})}
                              ...
                              <label className="flex items-center gap-2 cursor-pointer font-semibold text-slate-700">
                                <input 
                                  type="radio" 
                                  name="sched"
                                  checked={newCampaignForm.scheduledTime === "Schedule Later"}
                                  onChange={() => setNewCampaignForm({...newCampaignForm, scheduledTime: "Schedule Later"})}
                                  className="accent-emerald-500" 
                                />
                            ...
                            {newCampaignForm.scheduledTime === "Schedule Later" && (
                              <input 
                                type="datetime-local" 
                          ...
                          <div className="bg-slate-50 border border-slate-100 p-4 rounded-2xl space-y-3">
                            <h5 className="font-bold text-slate-800">Meta API Sending Rate Limiter</h5>
                            <div>
                              <label className="text-slate-400 font-semibold block mb-1">Delay interval between message packets</label>
                              <select 
                                value={newCampaignForm.sendingDelay}
                                onChange={(e) => setNewCampaignForm({...newCampaignForm, sendingDelay: e.target.value})}
                                className="w-full bg-white border border-slate-200 p-2 rounded-lg"
                              >
                                <option value="3-5 Seconds (Antispam Guard Enabled)">3-5 Seconds (Antispam Guard Enabled)</option>
                                <option value="Instant Blast (Danger: Meta Spam Alert Risk)">Instant Blast (Danger: Meta Spam Risk)</option>
                              </select>
                            </div>
                            
                            <div className="p-3 bg-amber-50 border border-amber-100 text-amber-800 rounded-xl leading-normal text-[10px]">
                              <strong>Anti-Spam Shield:</strong> Nilkamal CRM auto-throttles requests into staggered batches of 100/minute to protect your official Meta API credentials from suspensions.
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    {campaignWizardStep === 5 && (
                      <div className="space-y-4 text-xs">
                        <h4 className="font-extrabold text-slate-800 text-sm">Campaign Pre-Flight Compliance Checks</h4>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          {/* Left: Message preview bubble */}
                          <div className="space-y-2">
                            <span className="text-slate-400 font-bold block uppercase text-[10px]">What Customers Will See</span>
                            <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-3xl max-w-sm relative shadow-sm">
                              <div className="bg-white border border-slate-100 p-2 rounded-2xl mb-2 text-[10px] flex items-center gap-2">
                                ...
                                <div className="truncate">
                                  <strong className="block text-slate-800">{newCampaignForm.attachedMedia}</strong>
                                  <span className="text-slate-400 block text-[9px]">Ingested PDF Catalogue Attach</span>
                                </div>
                              </div>
                              <p className="text-xs text-slate-800 leading-relaxed font-medium">
                                {templates.find(t => t.id === newCampaignForm.selectedTemplateId)?.body
                                  .replace("{{customer_name}}", "Suresh Radhakrishnan")
                                  .replace("{{catalog_link}}", "nilkamal.com/promo-sale")
                                  .replace("{{city}}", "Mumbai")
                                  .replace("{{sales_executive}}", "Amit Sharma")
                                }
                              </p>
                          ...
                          <div className="bg-slate-50 border border-slate-100 p-4 rounded-3xl space-y-3">
                            ...
                            <div className="space-y-2 text-[11px] font-medium text-slate-600">
                              <div className="flex items-center gap-2">
                                ...
                                <span>Variables fully mapped to existing Lead properties.</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <CheckCircle className="w-4.5 h-4.5 text-emerald-500 shrink-0" />
                                <span>Unsubscribe button / STOP button option included in template.</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <CheckCircle className="w-4.5 h-4.5 text-emerald-500 shrink-0" />
                                <span>Media file formats compliant with Meta Cloud parameters.</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <CheckCircle className="w-4.5 h-4.5 text-emerald-500 shrink-0" />
                                <span>Daily limit parameters hold safely within Tier 2 margins.</span>
                              </div>
                            </div>
                          </div>
                        </div>
                    ...
                    {campaignWizardStep === 6 && (
                      <div className="space-y-4 text-xs text-center py-6">
                        ...
                        <div>
                          <h4 className="font-extrabold text-slate-900 text-base">You are Ready to Launch!</h4>
                          <p className="text-xs text-slate-500 mt-1">This will safely initiate bulk broadcast pipelines across your filtered recipient databases.</p>
                        </div>
                        ...
                        <div className="bg-slate-50 border border-slate-100 p-4 rounded-3xl max-w-md mx-auto grid grid-cols-2 gap-4 text-left font-medium text-slate-600">
                          <div>
                            <span className="text-slate-400 block uppercase font-bold text-[9px]">Campaign Target Name</span>
                            <strong className="text-slate-900 text-xs block mt-0.5">{newCampaignForm.name}</strong>
                          </div>
                          <div>
                            <span className="text-slate-400 block uppercase font-bold text-[9px]">Audience Blast Reach</span>
                            <strong className="text-slate-900 text-xs block mt-0.5">
                              {newCampaignForm.targetType === "all" ? "4,320 Verified Customers" : "1,850 Segmented Leads"}
                            </strong>
                          </div>
                          <div>
                            <span className="text-slate-400 block uppercase font-bold text-[9px]">API Endpoint Code</span>
                            <strong className="text-emerald-600 font-bold block mt-0.5">META_LIVE_OUTBOX</strong>
                          </div>
                          <div>
                            <span className="text-slate-400 block uppercase font-bold text-[9px]">Antispam Throttling</span>
                            <strong className="text-slate-800 text-xs block mt-0.5">Staggered (Delay engaged)</strong>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="pt-4 border-t border-slate-100 flex justify-between items-center text-xs">
                      <button
                        ...
                        onClick={() => setCampaignWizardStep(campaignWizardStep - 1)}
                      ...
                      {campaignWizardStep < 6 ? (
                        <button
                          type="button"
                          onClick={() => {
                            if (campaignWizardStep === 1 && newCampaignForm.targetType === "upload" && !draggedFile) {
                              showToast("Please drag & drop your Excel or CSV customer spreadsheet first.", "error");
                              return;
                            }
              ...
              {campaignTab === "templates" && (
                <div className="space-y-6">
                  <div className="flex justify-between items-center">
                    <div>
                      <h4 className="font-extrabold text-slate-900 text-sm">Meta Custom Inbound & Outbound Templates</h4>
                      <p className="text-xs text-slate-400">Pre-approved interactive formats to prevent spam filters on customer chats.</p>
                    </div>
                    <button
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {templates.map(tmp => (
                      <div key={tmp.id} className="bg-white border border-slate-100 p-5 rounded-3xl space-y-4 shadow-sm relative overflow-hidden">
                        <div className="flex justify-between items-start">
                          <div>
                            <span className="px-2 py-0.5 bg-slate-100 text-slate-600 font-bold text-[9px] uppercase rounded">
                              {tmp.category}
                            </span>
                            <h5 className="font-extrabold text-slate-900 text-sm mt-1">{tmp.name}</h5>
                            <span className="text-[10px] text-slate-400">ID: {tmp.id} • Language: {tmp.language}</span>
                          </div>
                        ...
                        <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100">
                          <p className="text-xs text-slate-600 leading-relaxed whitespace-pre-wrap">{tmp.body}</p>
                        </div>

                        <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-50">
                          {tmp.buttons.map((btn, bIdx) => (
                            <span key={bIdx} className="text-[10px] bg-white border border-slate-200 px-3 py-1.5 rounded-xl font-bold text-slate-700 shadow-xs">
                              Button: {btn}
                            </span>
                          ))}
                        </div>
                  ...
                  {isCreatingNewTemplate && (
                    <div className="fixed inset-0 bg-slate-950/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                      <div className="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl space-y-5 animate-modal-scale">
                        <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                          <h4 className="font-extrabold text-slate-900">Create Custom Template</h4>
                          <button onClick={() => setIsCreatingNewTemplate(false)} className="p-1 hover:bg-slate-100 rounded-lg text-slate-400">
                            <X className="w-5 h-5" />
                          </button>
                        </div>

                        <form onSubmit={createTemplate} className="space-y-4 text-xs">
                          <div className="space-y-1">
                            <label className="text-slate-400 font-bold block">Template Registry Name</label>
                            <input 
                              type="text" 
                          ...
                          <div className="space-y-1">
                            <label className="text-slate-400 font-bold block">Meta Category Class</label>
                            <select 
                          ...
                          <div className="space-y-1">
                            <label className="text-slate-400 font-bold block">Message Body Content</label>
                            <textarea 
                              rows={4} 
                          ...
                          <div className="p-3 bg-blue-50 text-blue-700 rounded-2xl leading-normal text-[10px]">
                            <strong>Variable Guidance:</strong> Use double curly braces `{{variable_name}}` to dynamically insert customer specifics during send-time execution.
                          </div>

                          <div className="pt-4 border-t border-slate-100 flex justify-end gap-2 text-xs">
                            <button type="button" onClick={() => setIsCreatingNewTemplate(false)} className="px-4 py-2 bg-slate-100 text-slate-600 rounded-xl">Cancel</button>
                            <button type="submit" className="px-4 py-2 bg-emerald-500 text-white font-bold rounded-xl">Submit for Meta API Approval</button>
                          </div>
                        </form>
                      </div>
                    </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white border border-slate-100 p-5 rounded-3xl shadow-sm space-y-4">
                      <div>
                        <h4 className="font-extrabold text-slate-900 text-sm">Conversion Funnel Heatmap</h4>
                        <p className="text-xs text-slate-400">Tracking aggregate drop-offs from bulk broadcasts.</p>
                      </div>

                      {/* Pure beautiful drawing representing conversion funnels */}
                      <div className="space-y-3.5 pt-4">
                        {[
                          { step: "Sent Broadcast", count: "100%", width: "w-full", color: "bg-blue-600" },
                          { step: "Delivered Webhook", count: "98.2%", width: "w-[98%]", color: "bg-teal-500" },
                          { step: "Reads Detected", count: "88.4%", width: "w-[88%]", color: "bg-indigo-500" },
                          { step: "Chats Back/Replied", count: "19.7%", width: "w-[20%]", color: "bg-emerald-500" },
                          { step: "Closed Converted Sale", count: "4.2%", width: "w-[8%]", color: "bg-purple-600" }
                        ].map((fn, idx) => (
                          <div key={idx} className="space-y-1 text-xs">
                            <div className="flex justify-between items-center text-[11px] font-bold text-slate-500">
                              <span>{fn.step}</span>
                              <span className="font-black text-slate-900">{fn.count}</span>
                            </div>
                            <div className="w-full bg-slate-100 h-6 rounded-lg overflow-hidden relative">
                              <div className={`${fn.color} h-full ${fn.width} rounded-lg`} />
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="bg-white border border-slate-100 p-5 rounded-3xl shadow-sm space-y-4 lg:col-span-2">
                      <div>
                        <h4 className="font-extrabold text-slate-900 text-sm">Real-time Webhook Response Heatmap</h4>
                        <p className="text-xs text-slate-400">Hourly response pingbacks over the last 24 hours.</p>
                      </div>
                      ...
                      <div className="h-44 w-full relative">
                        <svg className="w-full h-full overflow-visible" viewBox="0 0 500 120">
                          ...
                          <circle cx="120" cy="40" r="5" fill="#10b981" stroke="#ffffff" strokeWidth="2" />
                          <circle cx="360" cy="20" r="5" fill="#10b981" stroke="#ffffff" strokeWidth="2" />

                          <text x="10" y="115" fill="#94a3b8" fontSize="10px">09:00 AM</text>
                          <text x="150" y="115" fill="#94a3b8" fontSize="10px">01:00 PM</text>
                          <text x="300" y="115" fill="#94a3b8" fontSize="10px">05:00 PM</text>
                          <text x="450" y="115" fill="#94a3b8" fontSize="10px">09:00 PM</text>
                        </svg>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-900 text-white p-5 rounded-3xl space-y-4">
                    <div>
                      <h4 className="font-extrabold text-sm">Automated Real-Time Pipeline Diagram</h4>
                      <p className="text-xs text-slate-400">How message dispatch handles Meta cloud webhooks and assigns active responders.</p>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-4 bg-slate-800/40 rounded-2xl border border-dashed border-slate-700 text-xs font-bold text-center">
                      <div className="flex-1 bg-slate-800 p-3 rounded-xl border border-slate-700 space-y-1 text-xs">
                        ...
                        <span className="text-white block">1. Segment Filter Ingest</span>
                        <p className="text-[10px] text-slate-400 font-medium">Auto-filtering matching contacts based on purchase history.</p>
                      </div>
                      <span className="text-emerald-500 font-extrabold rotate-90 md:rotate-0">➔</span>
                      <div className="flex-1 bg-slate-800 p-3 rounded-xl border border-slate-700 space-y-1 text-xs">
                        <SlidersHorizontal className="w-5 h-5 text-sky-400 mx-auto animate-pulse" />
                        <span className="text-white block">2. Batch Throttling (Redis)</span>
                        <p className="text-[10px] text-slate-400 font-medium">Staggering package dispatches to avoid Meta anti-spam flags.</p>
                      </div>
                      <span className="text-emerald-500 font-extrabold rotate-90 md:rotate-0">➔</span>
                      <div className="flex-1 bg-slate-800 p-3 rounded-xl border border-slate-700 space-y-1 text-xs">
                        <Wifi className="w-5 h-5 text-emerald-400 mx-auto" />
                        <span className="text-white block">3. Meta Cloud APIs</span>
                        <p className="text-[10px] text-slate-400 font-medium">Meta dispatches messages directly to verified client devices.</p>
                      </div>
                      <span className="text-emerald-500 font-extrabold rotate-90 md:rotate-0">➔</span>
                      <div className="flex-1 bg-slate-800 p-3 rounded-xl border border-slate-700 space-y-1 text-xs">
                        <MessageSquare className="w-5 h-5 text-amber-400 mx-auto" />
                        <span className="text-white block">4. Hook Responder Sync</span>
                        <p className="text-[10px] text-slate-400 font-medium">Incoming responses auto-route straight into active agent chat desks.</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {campaignTab === "limits" && (
                <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm max-w-4xl mx-auto space-y-6 text-xs text-slate-600">
                  <div>
                    <h4 className="font-extrabold text-slate-900 text-base">Meta Cloud API Operational Limits</h4>
                    <p className="text-xs text-slate-400">Maintain high API status and quality ratings by matching guidelines.</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-slate-50 border border-slate-100 p-4 rounded-3xl space-y-3">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between border-b border-slate-200 pb-1.5 font-medium">
                          ...
                          <strong className="text-slate-900">Tier 2 (10,000 Messages/Day)</strong>
                        </div>
                        <div className="flex justify-between border-b border-slate-200 pb-1.5 font-medium">
                          <span>Active Quality Score:</span>
                          <strong className="text-emerald-600 font-extrabold">High (99.8% Approval)</strong>
                        </div>
                        <div className="flex justify-between font-medium">
                          <span>Registered Meta ID:</span>
                          <strong className="text-slate-700">WABA-MUMBAI-OFFICE-8812</strong>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-50 border border-slate-100 p-4 rounded-3xl space-y-3">
                      ...
                      <div>
                        <label className="text-slate-400 font-bold block mb-1">Store Daily Send-out Safeguard Cap</label>
                        <select className="w-full bg-white border border-slate-200 p-2 rounded-lg font-bold">
                          <option value="5000">Limit to 5,000 Messages/day</option>
                          <option value="10000">Limit to 10,000 Messages/day (No cap)</option>
                        </select>
                      </div>
                    </div>
                  </div>
              ...
              {selectedCampaignForDetail && (
                <div className="fixed inset-0 bg-slate-950/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                  <div className="bg-white w-full max-w-xl rounded-3xl p-6 shadow-2xl space-y-5 animate-modal-scale max-h-[90vh] overflow-y-auto text-xs text-slate-600">
                    <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                      <div>
                        <span className="text-[9px] font-extrabold text-blue-600 uppercase">Campaign Audit Report</span>
                        <h4 className="font-black text-slate-900 text-base mt-1">{selectedCampaignForDetail.name}</h4>
                      </div>
                      <button onClick={() => setSelectedCampaignForDetail(null)} className="p-1 hover:bg-slate-100 rounded-lg text-slate-400">
                        <X className="w-5 h-5" />
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 bg-slate-50/50 p-4 rounded-2xl border border-slate-100 font-medium">
                      <div>
                        <span className="text-slate-400 block text-[9px] uppercase font-bold">Sender Executive</span>
                        <strong className="text-slate-800 block mt-0.5">{selectedCampaignForDetail.createdBy}</strong>
                      </div>
                      <div>
                        <span className="text-slate-400 block text-[9px] uppercase font-bold">Execution Date</span>
                        <strong className="text-slate-800 block mt-0.5">{selectedCampaignForDetail.sentDate}</strong>
                      </div>
                      <div>
                        <span className="text-slate-400 block text-[9px] uppercase font-bold">Core Template Template Used</span>
                        <strong className="text-slate-800 block mt-0.5">{selectedCampaignForDetail.template}</strong>
                      </div>
                      <div>
                        <span className="text-slate-400 block text-[9px] uppercase font-bold">Audience Reach</span>
                        <strong className="text-slate-900 font-black block mt-0.5">{selectedCampaignForDetail.audienceSize.toLocaleString()} Recipient Leads</strong>
                      </div>
                    </div>

                    <div className="space-y-2">
                      <h5 className="font-bold text-slate-800 text-xs">Meta Cloud Delivery Breakdown</h5>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <div className="p-2.5 bg-slate-50 rounded-xl border border-slate-100">
                          <span className="text-slate-400 text-[9px] font-bold block uppercase">Delivered</span>
                          <strong className="text-slate-950 block text-sm font-black">{selectedCampaignForDetail.delivered.toLocaleString()}</strong>
                        </div>
                        <div className="p-2.5 bg-emerald-50 rounded-xl border border-emerald-100">
                          <span className="text-emerald-600 text-[9px] font-bold block uppercase">Delivery %</span>
                          <strong className="text-emerald-600 block text-sm font-black">{selectedCampaignForDetail.deliveryPercent || 0}%</strong>
                        </div>
                        <div className="p-2.5 bg-sky-50 rounded-xl border border-sky-100">
                          <span className="text-sky-600 text-[9px] font-bold block uppercase">Read Rate</span>
                          <strong className="text-sky-600 block text-sm font-black">{selectedCampaignForDetail.read.toLocaleString()}</strong>
                        </div>
                        <div className="p-2.5 bg-amber-50 rounded-xl border border-amber-100">
                          <span className="text-amber-600 text-[9px] font-bold block uppercase">Replies back</span>
                          <strong className="text-amber-600 block text-sm font-black">{selectedCampaignForDetail.replied.toLocaleString()}</strong>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="pt-4 border-t border-slate-100 flex justify-end gap-2">
                      <button
                        onClick={() => {
                          showToast("Exporting XLS audit logs to executive downloads.", "success");
                      ...
                      <button onClick={() => setSelectedCampaignForDetail(null)} className="px-4 py-2 bg-slate-900 text-white font-bold rounded-lg">
                        Close Report
                      </button>
                    </div>
                  </div>
                      ...
                      return (
                        <div key={emp.id} className="space-y-1.5">
                          <div className="flex justify-between items-center text-xs">
                            ...
                            <span className="font-semibold text-slate-500 font-mono">₹{emp.currentSales.toLocaleString('en-IN')} / {percentage}%</span>
                          </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex justify-between items-baseline mb-1">
                              <h4 className="font-bold text-xs text-slate-900 truncate">{chat.name}</h4>
                              <span className="text-[9px] text-slate-400 shrink-0 font-mono">{lastMsg ? lastMsg.time : "Now"}</span>
                            </div>
                            <p className="text-[11px] text-slate-500 truncate mt-0.5 leading-snug">
                              {lastMsg ? lastMsg.text : "Inquiry registered."}
                            </p>
                  ...
                  {activeChat ? (
                    <>
                      <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <span className="text-xs text-slate-400 block mt-0.5">Contact: {activeChat.phone}</span>
                          </div>
                        </div>
                      </div>
                        ...
                        {activeChat.messages.map((msg, idx) => {
                          ...
                          return (
                            <div key={idx} className={`flex ${isAgent ? "justify-end" : "justify-start"}`}>
                              <div className={`max-w-[75%] rounded-2xl p-3.5 shadow-sm text-xs ${
                                isAgent 
                                  ? "bg-slate-900 text-white rounded-tr-none" 
                                  : "bg-white text-slate-800 border border-slate-100 rounded-tl-none"
                              }`}>
                      ...
                      <form onSubmit={handleSendWhatsappMessage} className="p-4 border-t border-slate-100 flex gap-2 items-center bg-white">
                        ...
                        />
                        <button type="submit" className="p-3 bg-slate-900 text-white rounded-xl">
                          <Send className="w-4.5 h-4.5" />
                        </button>
                      </form>
                    </>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
                      ...
                      <p className="text-xs mt-2">Choose an active client queue to start chat.</p>
                    </div>
                  )}
                </div>
                ...
                <div className="lg:col-span-1 border-l border-slate-100 p-4 space-y-4">
                  {activeChat ? (
                    <div className="space-y-4 text-xs">
                      ...
                      <div>
                        <span className="text-slate-400 font-bold block mb-1 uppercase text-[9px]">Budget</span>
                        <strong className="text-slate-900 block text-sm font-mono">₹{activeChat.budget.toLocaleString('en-IN')}</strong>
                      </div>
                      <div>
                        <span className="text-slate-400 font-bold block mb-1 uppercase text-[9px]">Assign Special Desk</span>
                        <select
                    ...
                    <tbody className="divide-y divide-slate-50 text-xs text-slate-600">
                      {filteredLeads.map((lead) => (
                        <tr key={lead.id} className="hover:bg-slate-50/40 transition-colors">
                          ...
                          <td className="p-4 space-y-1">
                            <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold ${
                              lead.priority === "Hot" ? "bg-rose-50 text-rose-600" : "bg-amber-50 text-amber-600"
                            }`}>
                              {lead.priority}
                            </span>
                          </td>
                    ...
                    >
                      ...
                      <div className="space-y-1 pt-2 border-t border-slate-50">
                        <div className="flex justify-between items-center text-[10px] font-bold text-slate-400">
                          <span>Progress</span>
                          <span className="text-slate-800 font-mono">{percentage}%</span>
                        </div>
                        <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                ...
                <div className="bg-white border border-slate-100 p-5 rounded-2xl shadow-sm space-y-4">
                  ...
                  <div className="space-y-3 max-h-[400px] overflow-y-auto">
                    {leads.filter(l => {
                      ...
                      <div key={lead.id} className="p-3 bg-rose-50/50 border border-rose-100 rounded-xl space-y-2 text-xs">
                        <div className="flex justify-between items-start">
                          <div>
                  ...
                  <div className="space-y-3.5 text-xs">
                    {leads.filter(l => {
                      ...
                      <div key={lead.id} className="flex items-center justify-between p-3 hover:bg-slate-50 border border-slate-100/50 rounded-xl">
                        ...
                        <div className="flex items-center gap-4">
                          <span className="text-xs font-semibold text-slate-700 font-mono">{lead.followUpDate}</span>
                          <button 
                            onClick={() => setSelectedLeadForDetail(lead)}
                            className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-400"
                          >
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs">
                ...
                <div className="bg-white border border-slate-100 p-5 rounded-2xl shadow-sm space-y-4">
                  <h3 className="font-bold text-slate-900 text-sm">Revenue Share by Source</h3>
                  {[
                    { label: "WhatsApp Campaigns", revenue: "₹4,10,000", pct: 41 },
                    { label: "Walk-in Showroom", revenue: "₹2,40,000", pct: 24 },
                    { label: "Instagram Ads", revenue: "₹1,80,000", pct: 18 },
                    { label: "Phone & B2B Inquiry", revenue: "₹1,70,000", pct: 17 }
                  ].map((source, idx) => (
                    <div key={idx} className="flex justify-between items-center">
                      ...
                      <span className="font-extrabold text-slate-900 font-mono">{source.revenue}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5 text-xs">
                {leads.filter(l => l.status === "Converted").map(lead => (
                  <div key={lead.id} className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm space-y-4 relative overflow-hidden">
                    ...
                    <div>
                      <span className="text-slate-400 font-bold block text-[9px]">Order Reference ID</span>
                      <strong className="text-slate-900 text-sm mt-0.5 block">{lead.name}</strong>
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-50 font-mono font-bold">
                      <div>
                        <span className="text-slate-400 text-[9px] block">Order Value</span>
                        <span>₹{lead.budget.toLocaleString('en-IN')}</span>
                      </div>
                      <div>
                        <span className="text-slate-400 text-[9px] block">Executive Desk</span>
          ...
          {activeTab === "settings" && (
            <div className="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm space-y-6 max-w-4xl text-xs">
              <div>
                <h2 className="text-xl font-black text-slate-900 tracking-tight">Nilkamal CRM Config</h2>
                <p className="text-slate-400">Configure parameters, role permissions, and customized lead source hooks.</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <h3 className="font-bold text-slate-800 text-sm">General Details</h3>
                  <div className="space-y-2">
                    <label className="text-slate-400 font-semibold block">Store Location Branch ID</label>
                    <input type="text" className="w-full p-2.5 bg-slate-50 border border-slate-100 rounded-lg text-slate-700 font-semibold" value="NK-WEST-MUM-01" readOnly />
                  </div>
                </div>

                <div className="space-y-4">
                  <h3 className="font-bold text-slate-800 text-sm">Connected API Gateway Key</h3>
                  <div className="space-y-2">
                    <label className="text-slate-400 font-semibold block">Meta Sandbox API Access Token</label>
                    <input type="password" value="••••••••••••••••••••••••••••••••" className="w-full p-2.5 bg-slate-50 border border-slate-100 rounded-lg text-slate-400" readOnly />
                  </div>
                </div>
              </div>
            </div>
      ...
      {selectedLeadForDetail && (
        <div className="fixed inset-0 bg-slate-950/20 backdrop-blur-sm z-50 flex justify-end">
          <div className="w-full max-w-lg bg-white h-full shadow-2xl p-6 overflow-y-auto flex flex-col justify-between animate-slide-in-right text-xs">
            <div className="space-y-6">
              <div className="flex justify-between items-start pb-4 border-b border-slate-100">
                <div>
                  <span className="text-[10px] font-bold text-blue-600 uppercase tracking-wide">Lead Context</span>
                  <h3 className="text-xl font-black text-slate-950 tracking-tight mt-1">{selectedLeadForDetail.name}</h3>
                  <span className="text-slate-400 font-mono">ID: {selectedLeadForDetail.id}</span>
                </div>
                <button onClick={() => setSelectedLeadForDetail(null)} className="p-1 hover:bg-slate-100 rounded-lg text-slate-400">
                  <X className="w-5 h-5" />
                </button>
              </div>

              <div className="space-y-2">
                <span className="text-slate-400 font-bold block">Update Status</span>
                <div className="flex flex-wrap gap-1.5">
                  {["New", "Contacted", "Interested", "Follow-up Pending", "Negotiation", "Converted", "Lost"].map(statusVal => {
                    const isActive = selectedLeadForDetail.status === statusVal;
                    return (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all ${
                          isActive ? "bg-blue-600 text-white" : "bg-slate-50 hover:bg-slate-100 text-slate-600"
                        }`}
                      >
                        {statusVal}
                      </button>
            ...
            <div className="pt-4 border-t border-slate-100">
              <button onClick={() => setSelectedLeadForDetail(null)} className="w-full py-2 bg-slate-950 text-white font-bold rounded-lg text-xs">
                Save & Close Detail
              </button>
            </div>
          </div>
      ...
      {isAddLeadOpen && (
        <div className="fixed inset-0 bg-slate-950/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-3xl p-6 shadow-2xl space-y-5 animate-modal-scale max-h-[90vh] overflow-y-auto text-xs text-slate-600">
            <div className="flex justify-between items-center pb-3 border-b border-slate-100">
              <h3 className="text-base font-extrabold text-slate-900">Add New Furniture CRM Lead</h3>
              <button onClick={() => setIsAddLeadOpen(false)} className="p-1 hover:bg-slate-100 rounded-lg text-slate-400">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateLead} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input 
                    ...
                    value={newLeadForm.name}
                    onChange={(e) => setNewLeadForm({...newLeadForm, name: e.target.value})}
                    className="w-full p-2 bg-slate-50 border border-slate-100 rounded-lg" 
                  />
                </div>
                <div className="space-y-1">
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-end gap-2 text-xs">
                <button type="button" onClick={() => setIsAddLeadOpen(false)} className="px-4 py-2 bg-slate-50 text-slate-600 rounded-lg">Cancel</button>
                <button type="submit" className="px-4 py-2 bg-slate-900 text-white font-bold rounded-lg">Create Lead</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-950/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl space-y-4 animate-modal-scale text-xs text-slate-600">
            ...
            <div className="space-y-3.5">
              <div className="space-y-1">
                <label className="text-slate-400 font-semibold">Full Name</label>
                <input id="newEmpName" type="text" placeholder="e.g. Suresh Kumar" className="w-full p-2.5 bg-slate-50 border border-slate-100 rounded-lg" />
              </div>
            </div>

            <div className="pt-4 border-t border-slate-100 flex justify-end gap-2 text-xs">
              <button onClick={() => setIsAddEmployeeOpen(false)} className="px-4 py-2 bg-slate-100 text-slate-600 rounded-lg">Cancel</button>
              <button 
                onClick={() => {
                  const name = document.getElementById("newEmpName")?.value;
                  if (!name) return;
                  const newEmp = {
                    id: `EMP-0${employees.length + 1}`,
                    name, role: "Sales Executive", activeLeads: 0, convertedLeads: 0,
                    target: 400000, currentSales: 0, status: "Active", avatar: "SK"
                  };
                  setEmployees([...employees, newEmp]);
                  setIsAddEmployeeOpen(false);
